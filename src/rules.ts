/**
 * Validation Rule class and a set of reusable built-in rules.
 *
 * A `Rule` implements the `Validator` interface (see `types.ts`) and adds
 * fluent helpers for customizing the message and attaching a conditional
 * predicate.
 */
...
export class Rule {
    ...
    /** Return a copy of this rule with a custom error message. */
    withMessage(message: string): Rule {
        return new Rule(this.type, message, this.validate, this.condition);
    }

    /**
     * Return a copy of this rule that only runs when `predicate(data)` is
     * truthy. Useful for cross-field conditional validation.
     */
    when(predicate: (data: Record<string, any>) => boolean): Rule {
        return new Rule(this.type, this.message, this.validate, predicate);
    }
}

/** Reusable, type-safe built-in rules. */
